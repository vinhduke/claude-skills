...
/// Menu item ID for toggling hover-only mode (used by window.rs context menu).
pub const IDM_TOGGLE_HOVER_MODE: u16 = 51;
/// Menu item ID for toggling the standalone always-on-top window.
pub const IDM_TOGGLE_STANDALONE_WINDOW: u16 = 52;
...
/// `wparam` carries the tray icon ID (NOTIFYICONDATAW.uID).
pub fn handle_message(wparam: WPARAM, lparam: LPARAM) -> TrayAction {
    let mouse_msg = lparam.0 as u32;
    let kind = TrayIconKind::from_id(wparam.0 as u32);
    match mouse_msg {
        WM_LBUTTONUP => TrayAction::ToggleWidget,
        WM_RBUTTONUP => TrayAction::ShowContextMenu,
        WM_MOUSEMOVE => match kind {
            Some(k) => TrayAction::HoverMove(k),
...
/// Screen rect of the given tray icon, or None if the shell can't locate it
/// (e.g. icon is hidden in an overflow flyout or not yet realized).
pub fn get_icon_screen_rect(hwnd: HWND, kind: TrayIconKind) -> Option<RECT> {
    unsafe {
        ...
        match Shell_NotifyIconGetRect(&nid) {
            Ok(rect) => Some(rect),
            Err(_) => None,
        }
    }
